...
//         let val = ''
//         { val = props.props.fav.favorites.filter(itm => item.id == itm.ad_id && itm.user_id == userId).map((item, index) => { return (item.ad_id) }) }
//         return (
...
class adDetail extends Component {
  ...
  renderRelatedAds(catId, id) {
    if (this.props.ads.isLoading) {
      ...
      return (
        // <Text>{JSON.stringify(props.props)}</Text>
        this.props.ads.ads.filter(item => item.category_id == catId && item.id != id).map((item, index) => {
          return (
            <Card containerStyle={styles.productCardColumn} key={index}>
